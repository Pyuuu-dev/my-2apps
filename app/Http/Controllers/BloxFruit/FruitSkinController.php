...
class FruitSkinController extends Controller
{
    public function index(Request $request)
    {
        ...
        $gamepassesForCopy = Gamepass::withSum('stocks as stok', 'jumlah')->where('aktif', true)->orderByDesc('harga_jual')->get()->map(fn($g) => ['nama' => $g->nama, 'harga_jual' => $g->harga_jual, 'stok' => (int)($g->stok ?? 0)]);
        $permanentsForCopy = PermanentFruitPrice::withSum('stocks as stok', 'jumlah')->where('aktif', true)->orderByDesc('harga_jual')->get()->map(fn($p) => ['nama' => $p->nama, 'harga_jual' => $p->harga_jual, 'stok' => (int)($p->stok ?? 0)]);
