...
class StoreSettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        foreach ($values as $key => $value) {
            ...
            // Skip semua validasi kalau nilai tidak berubah dari yang ada di DB.
            // Mencegah field legacy/stale memblokir penyimpanan field lain yang ingin user update.
            $unchanged = is_string($value) && $value === (string) $existing->value;

            // URL fields: only allow http(s) scheme — blocks javascript:, data:, file: etc.
            if (!$unchanged && $existing->type === 'url' && $value !== '') {
            ...
            if (!$unchanged && $key === 'store.wa_number' && $value !== '') {
