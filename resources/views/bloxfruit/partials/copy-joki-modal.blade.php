{{--
    Copy Joki Promo Modal (reusable)
    Include this inside an Alpine x-data component (jokiPromoPage) that has:
    - showCopy (boolean)
    - sections (object: { header: bool, kat_<kategori>: bool ... })
    - includeKeterangan (boolean)
    - jokiData (array dari controller: kategori/label/icon/items)
    - headerText (string)
    - generatedText (getter)
    - copyText() / resetHeader()
--}}
<div x-show="showCopy" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" @keydown.escape.window="showCopy = false">
    ...
    <div class="relative w-full max-w-2xl max-h-[85vh] rounded-2xl bg-white dark:bg-slate-900 shadow-2xl overflow-hidden flex flex-col">
        ...
        <div class="flex items-center justify-between px-5 py-3 border-b border-gray-100 dark:border-slate-700 shrink-0">
            <h3 class="font-bold text-gray-900 dark:text-white">Copy Teks Promo Joki</h3>
        ...
        <div class="px-5 py-3 border-b border-gray-100 dark:border-slate-700 shrink-0 space-y-3">
            <div class="flex flex-wrap gap-2">
                ...
                <label class="flex items-center gap-1.5 text-xs">
                    <input type="checkbox" x-model="includeKeterangan" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"> Sertakan keterangan
                </label>
            </div>

            {{-- Dynamic checkbox per kategori --}}
            <div class="flex flex-wrap gap-2">
                <template x-for="cat in jokiData" :key="cat.kategori">
                    <label class="flex items-center gap-1.5 text-xs">
                        <input type="checkbox"
                               :checked="sections['kat_' + cat.kategori]"
                               @change="sections['kat_' + cat.kategori] = $event.target.checked"
            ...
            <div x-show="sections.header" x-collapse>
                <div class="flex items-center justify-between mb-1">
                    <label class="block text-[10px] font-semibold text-gray-500">Edit Header:</label>
                    <button @click="resetHeader()" type="button" class="text-[10px] text-indigo-600 hover:text-indigo-800">Reset default</button>
                </div>
